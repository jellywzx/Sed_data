#!/usr/bin/env python3
"""Temporary stats: Q/SSC/SSL value distribution across per-station USGS qc NetCDF files.

Methodology mirrors Script/GFQA_v2/tmp_stats_distribution.py (which follows
Output_r/scripts_basin_test/stats_release/variable_summary.py conventions),
adapted to per-station USGS files:
  - daily: Output_r/daily/USGS/qc/*.nc (890 stations)

Note on flag semantics: SSL is derived (Q*SSC*0.0864); the QC propagates
"all inputs good" as flag 1 (estimated), so SSL never has flag 0.
n_good=0 / n_estimated=~all for SSL is expected, not a QC failure.
"""
...
PROJECT_ROOT = SCRIPT_DIR.parents[1]  # Script/USGS -> sediment_wzx_1111
DIRS = {
    "daily": PROJECT_ROOT / "Output_r" / "daily" / "USGS" / "qc",
}
...
def main():
    ...
    # ---- console ----
    print("=" * 104)
    print("USGS Q/SSC/SSL value distribution (pooled across stations)")
    print("=" * 104)
