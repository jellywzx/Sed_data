...
SOURCE_DATA_DIR = os.fspath(resolve_source_root(start=__file__) / "Myanmar")
TARGET_NC_DIR = os.fspath(
    resolve_output_root(start=__file__) / "daily" / "Myanmar" / "qc"
)
...
def create_netcdf_file(filepath, df, station_meta):
    ...
    with nc.Dataset(filepath, 'w', format='NETCDF4') as ds:
        ...
        ds.observation_type = "In-situ station data"
        ...
        ds.number_of_observations = str(len(df))
