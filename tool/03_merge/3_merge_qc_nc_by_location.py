...
OUTPUT_R_ROOT = _SCRIPT_DIR.parent.parent.parent / "Output_r"  # 数据根目录
...
BUILDIN_CONFIG = {
    ...
    "load_nc_threads": 4,            # 每个 cluster 内并行读 NC 的线程数（1=顺序读，2~8 可加速多站 cluster）
    "merge_chunksize": 1,           # 合并阶段每批任务数，1=每 cluster 一任务、负载最匀
    "dry_run": False,
    "only_collect": False,
...
def merge_series_keep_overlaps(records):
    ...
    has_val = combined["Q"].notna() | combined["SSC"].notna() | combined["SSL"].notna()
    combined = combined.loc[has_val].copy()
    if len(combined) == 0:
        return pd.DataFrame(columns=["date", "Q", "SSC", "SSL", "source", "is_overlap"])
    cnt = combined.groupby("date", sort=False).size().rename("_n")
    combined = combined.merge(cnt, left_on="date", right_index=True, how="left")
    combined["is_overlap"] = combined["_n"] > 1
    combined = combined.drop(columns=["_n"]).rename(columns={"_source": "source"})
    covered = set(combined["date"].unique())
    missing = sorted(all_dates - covered)
    if missing:
        extra = pd.DataFrame({
            "date": missing, "Q": np.nan, "SSC": np.nan, "SSL": np.nan,
            "source": "", "is_overlap": False,
        })
...
def merge_series_single_per_date(records):
    """
    合并多源时间序列：每个日期只保留一个值（按 records 顺序取第一个有任意非空的源的那一行）。
    返回 DataFrame：date, Q, SSC, SSL，用于写入合并后的 NetCDF。
    向量化：concat 后按 date 取第一个“有任意非空”的行。
    """
    pieces = []
    for source, df in records:
        if df is None or len(df) == 0:
            continue
        pieces.append(df[["date", "Q", "SSC", "SSL"]].copy())
    if not pieces:
        return None
    combined = pd.concat(pieces, ignore_index=True)
    has_any = combined["Q"].notna() | combined["SSC"].notna() | combined["SSL"].notna()
    first_per_date = combined.loc[has_any].drop_duplicates(subset=["date"], keep="first")
    all_dates = sorted(combined["date"].unique())
    out = first_per_date.set_index("date").reindex(all_dates).reset_index()
    return out[["date", "Q", "SSC", "SSL"]].sort_values("date").reset_index(drop=True)
...
def _load_one_nc(source_path):
    """供线程池调用：读一个 NC，返回 (source, df) 或 None。"""
...
def _merge_one_cluster(args):
    ...
    n_threads = BUILDIN_CONFIG.get("load_nc_threads", 1)
    records_with_df = []
    if n_threads and n_threads > 1 and len(recs) > 1:
        with ThreadPoolExecutor(max_workers=min(n_threads, len(recs))) as ex:
            for res in ex.map(_load_one_nc, recs):
                if res is not None:
                    records_with_df.append(res)
    else:
...
def main():
    ...
    if not use_pool:
        ...
        chunksize = max(1, c.get("merge_chunksize", 1))  # 1=每 cluster 一任务，负载最匀
